...
public static class PromptTemplates
{
    ...
    Respond with VALID JSON ONLY that matches EXACTLY this schema and keys:
    {
      ""research_brief"": ""<one comprehensive paragraph capturing the formal brief>"",
      ""objectives"": [""<objective 1>"", ""<objective 2>"", ""<objective 3>""],
      ""scope"": ""<string describing scope constraints and boundaries>""
    }

    Rules:
    - Keys must be exactly: research_brief, objectives, scope
    - Do NOT include extra keys (e.g., question, preferences).
    - ""objectives"" must be an array of strings.
    - ""scope"" must be a single string. If multiple scope items exist, join them in a single coherent sentence.
    ";
    ...
    Based on all the research in your knowledge base, create a comprehensive, well-structured answer to the overall research brief:
    <Research Brief>
    {research_brief}
    </Research Brief>
    ...
    /// </summary>
    public static string ReportGenerationDraftInsightPrompt => @"Based on all the research conducted and draft report, create a comprehensive, well-structured answer to the overall research brief:
    <Research Brief>
    {research_brief}
    </Research Brief>
    ...
    **List of All Relevant Sources (with citations in the report)**
    ...
    Synthesize the research findings on: {research_query}

    Reasearh Topic: {research_topic}
    ...
    **List of All Relevant Sources (with citations in the report)**
    ...
    /// </summary>
    public static string LeadResearcherMultipleStepsDiffusionDoubleCheckPrompt => @"
    ...
    2. **RefineDraftReport**: Refine draft report using the findings from ConductResearch
    3. **ResearchComplete**: Indicate that research is complete
    4. **ThinkTool**: For reflection and strategic planning during research

    **CRITICAL: Use ThinkTool before calling ConductResearch or refine_draft_report to plan your approach, and after each ConductResearch or refine_draft_report to assess progress**
    ...
    **Task Delegation Budgets** (Prevent excessive delegation):
    ...
    - **Limit tool calls** - Always stop after {max_researcher_iterations} tool calls to ThinkTool and ConductResearch if you cannot find the right sources
    </Hard Limits>
...
";
    ...
    1. **SearCrawl4AI**: For conducting web searches to gather information
